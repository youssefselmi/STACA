...
$req1= $db->query("SELECT sum(prix) as total,YEAR(date_commande) as d from commande GROUP BY YEAR(date_commande)");
$req1->execute();
               $liste= $req1->fetchALL(PDO::FETCH_OBJ);
               
  $req2= $db->query("SELECT sum(prix) as total,YEAR(date_commande) as d from commande GROUP BY YEAR(date_commande)");
$req2->execute();
               $liste2= $req2->fetchALL(PDO::FETCH_OBJ);


 $req3= $db->query("SELECT sum(prix) as total,YEAR(date_commande) as d from commande GROUP BY YEAR(date_commande) ");
 $req3->execute();
...
foreach ($liste as $tache) {
 $dataPoints1[] = array('y' => $tache->total, 'label' => $tache->d);
...
foreach ($liste2 as $tache) {
 $dataPoints2[] = array('y' => $tache->total, 'label' => $tache->d);
 
}
$dataPoints3 = array();
foreach ($liste3 as $tache) {
 $dataPoints3[] = array('y' => $tache->total, 'label' => $tache->d);
