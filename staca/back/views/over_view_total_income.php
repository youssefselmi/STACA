...
$db=config::getConnexion();
   ...
   $req1= $db->query("SELECT sum(prix) as prix_total, DATE_FORMAT(date_commande,'%b') as date_de_comm from sales where etat='delivered' GROUP BY DATE_FORMAT(date_commande,'%b') ");
   $req1->execute();
...
foreach ($liste as $row) {
    $dataPoints[] = array('label' => $row->date_de_comm, 'y' => $row->prix_total);
}
?>
