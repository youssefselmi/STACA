...
$db=config::getConnexion();
   ...
   $req1= $db->query("SELECT sum(qte) as tot,id_produit as product, DAYNAME(date_vente) as date_de_vente from ventes_produits GROUP BY DAYNAME(date_vente)");
   $req1->execute();
...
foreach ($liste as $row) {
    $dataPoints[] = array('label' => $row->date_de_vente, 'y' => $row->tot);
}
?>
